...
exports.name = name;
// Exposed for unit tests of prototype methods that need no live data source.
exports.MsSQL = MsSQL;
...
MsSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  if (transaction && transaction.connector === this) {
    if (transaction.connection) {
      ...
      // The transaction was already committed or rolled back. The statement runs on the pool as before, so no
      // caller breaks, but it is committed on its own and not as part of the transaction the caller may believe
      // it is in (PRV-7808). Warn so such call sites can be found in the logs and fixed.
      g.warn('loopback-connector-mssql: statement issued with an ended transaction (%s) runs outside it, ' +
        'on the pool, and is committed on its own. SQL: %s. If the transaction ended on purpose, delete ' +
        '`options.transaction` after commit or rollback before issuing more statements. If it ended ' +
        'unexpectedly, the flow must stop instead of continuing to write.', transaction.id, sql.slice(0, 200));
    }
  }
