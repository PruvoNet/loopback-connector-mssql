...
function attempt(transaction, operation, done, waitedOn) {
  transaction[operation](function(err) {
    ...
    if (err.code === 'EREQINPROG' && transaction._activeRequest !== waitedOn) {
    ...
    if (err.code === 'EREQINPROG') {
      debug('%s still refused by the same request after the wait gave up, reporting it to the caller', operation);
      return done(err);
    }
...
// cancel that request because it outlived the deadline. The wait is terminal in every case:
